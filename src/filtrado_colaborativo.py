...
tests = pd.read_csv(test_path,header=0, sep=",",error_bad_lines=False)[0:10]
...
def model(user_based,types,type_cal):
    ...
    return pd.DataFrame(tests.apply(lambda x: algo.predict(x[0],x[1]), axis=1))
    ...
    #return #test_predictions#[['userid','rating','recommendationtype','model']]
